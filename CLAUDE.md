...
3. **Baseline selection (Trends / Protocol / Research / CSV / SyncCoordinator):** prefer 14-day primary, fall back to 7-day. 30-day is *stable context only*, never the active comparator. **Sleep dashboard only** uses `BaselineEngine.selectDashboardBaseline(...)` — 30-day primary, 60-day fallback, minimum 5 valid nights (`dashboardMinimumValidNights`). Dashboard ignores `UserProfile.baselineWindowDays`. Copy in that view refers to "your usual sleep", not a specific window length. `SleepDashboardViewModel` fetches 60 recent sessions (up from 30) to cover this wider window.
4. **Sleep quality score formula** (`HealthSleepScoreEstimator` in `Core/Models/SleepModels.swift`):
   - **detailedStages / mixedSources:** Duration 40 pts (actual ÷ goal) + Bedtime 25 pts (circular deviation from baseline bedtime; 0 if <5 valid baseline nights; travel-exempt = full 25 pts) + Continuity 15 pts (WASO grace at 5 min, full penalty at 60 min) + Restorative 20 pts (deep+REM ratio vs baseline ratio; fallback to 40 % absolute target when baseline not ready). Total max = 100.
   - **unspecifiedSleepOnly (isPartial = true):** Duration 50 pts + Continuity 30 pts + Restorative 0 pts. Total max = 80 pts. Bedtime component is 0 (maxPts=0 passed for partial). `isPartial = true` is set on the score.
   - Bedtime component unlocks after ≥5 valid baseline nights. Never coerce travel=nil to no-exemption (invariant #2).
